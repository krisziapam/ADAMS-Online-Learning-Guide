...
function initQuiz(){
 ...
 function draw(){
  ...
  root.innerHTML=`<div class="reviewer-shell">
    ...
    <div class="reviewer-stats"><span>Mode: ${esc(labelFor(review.mode))}</span><span>Question ${review.index+1} of ${list.length}</span><span>Score: ${review.score} / ${review.attempts}</span></div>
    <div class="reviewer-progress"><div class="reviewer-fill" style="width:${Math.min(100,pct)}%"></div></div>
    <div class="reviewer-question"><span class="tag">${esc(item.category)}</span><h2>${esc(item.q)}</h2></div>
    <div class="reviewer-options">${options.map((opt,i)=>`<button class="reviewer-option" data-choice="${i}">${String.fromCharCode(65+i)}. ${item.isCode?`<code>${esc(opt)}</code>`:esc(opt)}</button>`).join('')}</div>
 ...
 function check(btn, chosen, item){
  ...
  fb.innerHTML=`<b>${ok?'Correct!':'Incorrect.'}</b><br><b>Answer:</b> ${item.isCode?`<pre><code>${esc(item.answer)}</code></pre>`:esc(item.answer)}<br><b>Why:</b> ${esc(item.explain)}`;
  const stats=root.querySelector('.reviewer-stats');
  if(stats) stats.innerHTML=`<span>Mode: ${esc(labelFor(review.mode))}</span><span>Question ${review.index+1} of ${currentList().length}</span><span>Score: ${review.score} / ${review.attempts}</span>`;
 }
 resetOrder(); draw();
}
